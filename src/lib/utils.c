...
void raise_http_request(char *hostname, char *port, char *path, char *req_text, char *method, char *resp_buffer, int max_recv_len) {
    ...
    if (sockfd < 0) {
        perror("[raise_http_request] socket creation failed!\n");
        exit(1);
    }
    ...
    if (req == NULL) {
        perror("[raise_http_request] error getting memory! seems like a resource crunch!\n");
        exit(-1);
    }
    ...
    if (send(sockfd, req, strlen(req), 0) < 0) {
        perror("[raise_http_request] error sending request to host!");
        close(sockfd);
        free(req);
        exit(8);
    }
    ...
    if (bytes_received < 0) {
        perror("[raise_http_request] receive failed!\n");
        close(sockfd);
        exit(-1);
    }
    close(sockfd);
